...
class ExperimentValue:
    ...
    def _format(self):
        ...
        if sigma_first < 4:
            sigma_main = round(sigma_main * 10) / 10
        ...
        if lg:
            return ((pattern + 'e{:d}').format(temp, lg),
                    (pattern + 'e{:d}').format(sigma_main, lg))
        else:
    ...
    def __str__(self):
        ...
        return pattern.format(val, self.unit, sig, self.unit,
                              self.epsilon * 100)
